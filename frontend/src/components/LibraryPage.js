...
import { Library, Gamepad2, Clock, PlayCircle, Trash2, RotateCcw } from "lucide-react";
        ...
        {roms.length === 0 ? (
          ...
          <div className="space-y-2">
            {roms.map((rom, idx) => (
              <div
                key={rom.name}
                data-testid={`library-rom-${idx}`}
                className="bg-[#141417] border border-white/5 rounded-xl px-5 py-4 flex items-center gap-4 hover:border-white/10 transition-all group"
              >
                {/* Icon */}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    onClick={(e) => removeRom(rom.name, e)}
            ...
            <div className="mt-4 p-4 bg-[#141417] border border-white/5 rounded-xl">
              <p className="text-gray-600 text-xs leading-relaxed">
                <span className="text-gray-400 font-semibold">Re-loading a ROM:</span> Since browsers can't access files from previous sessions for security reasons, clicking "Play" will ask you to re-select the file. Your play count and history are always saved.
              </p>
            </div>
          </div>
        )}
